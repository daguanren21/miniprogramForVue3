      ...
      <nut-form class="flex-1 overflow-auto" :model-value="form" ref="ruleForm">
        ...
        <nut-form-item class="jx-form-item" required label="抢救开始时间">
          <nut-input
            class="nut-input-text"
            :border="false"
            v-model="form.rescueBeginTime"
            @click="datePop.open('rescueBeginTime')"
            readonly
            placeholder="请选择开始时间"
            type="text"
          >
        ...
        <nut-form-item class="jx-form-item" required label="抢救结束时间">
          <nut-input
            class="nut-input-text"
            :border="false"
            v-model="form.rescueEndTime"
            @click="datePop.open('rescueEndTime')"
            placeholder="请选择结束时间"
            readonly
            type="text"
          >
        ...
        <nut-cell-group title="患者信息" v-if="form.useRescued === 'true'">
          <nut-form-item class="jx-form-item" label="救助成功">
            <nut-radio-group
              ...
              v-model="form.successfulRescued"
            >
              <nut-radio label="true">是</nut-radio>
              <nut-radio label="false">否</nut-radio>
            </nut-radio-group>
          ...
          <nut-form-item class="jx-form-item" label="基础病">
            <nut-input
              ...
              class="nut-input-text"
              max-length="50"
              v-model="form.patientUnderlyingDisease"
              placeholder="请输入基础疾病"
              type="text"
            >
            </nut-input>
          ...
          <nut-form-item class="jx-form-item" label="晕倒原因">
            <nut-textarea
              ...
              placeholder="请输入晕倒原因"
              v-model="form.patientEventTrigger"
              limit-show
              max-length="200"
            />
          </nut-form-item>
...
const confirm = async () => {
  ...
  try {
    ...
    await updateDeviceRescueData({
      ...
      successfulRescued: isResued
        ? form.successfulRescued === "false"
          ? false
          : true
        : null,
